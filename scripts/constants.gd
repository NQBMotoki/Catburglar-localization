...
func get_level_name(which : int) -> String:
	return tr(LEVEL_DETAILS[which]["name"])

func get_level_briefing(which : int) -> String:
	return tr(LEVEL_DETAILS[which]["briefing"])

func get_level_objectives(which : int) -> String:
	return tr(LEVEL_DETAILS[which]["objectives"])

func get_level_vo_path(which : int) -> String:
	var path : String = LEVEL_DETAILS[which]["vo"]
	if Settings.is_chinese():
		path = path.replace("audio/vo/", "audio/vo/zh/")
	return path
